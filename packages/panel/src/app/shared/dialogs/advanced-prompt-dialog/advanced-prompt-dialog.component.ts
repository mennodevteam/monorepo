  ...
  submit(): void {
    if (this.form.valid) {
      ...
      for (const key in this.fields) {
        if (Object.prototype.hasOwnProperty.call(this.fields, key)) {
          const field = this.fields[key];
          if (field.type === 'number') value[key] = Number(value[key]);
          else if (field.type === 'datepicker') value[key] = value[key] ? value[key]._d || value[key] : undefined;
        }
      }
      console.log(value);
