...
export class MemberDialogComponent implements OnInit {
  ...
  load() {
    this.club.getMemberByMobilePhone(this.data.mobilePhone).subscribe((member) => {
      if (member) {
        ...
        this.orderService.filter({ customerId: this.member.userId }).then((orders) => {
          this.orders = orders;
        });

      } else {
        this.snack.open(this.translate.instant('memberDialog.notMemberWarning'), '', { panelClass: 'warning' });
  ...
  async openSmsDialog() {
    const template: SmsTemplate = await this.dialog.open(MessageTemplateSelectorDialogComponent, {
      width: '960px',
  ...
  async publicKeyClick() {
    if (this.member.publicKey) {
      const ok = await this.dialog.open(AlertDialogComponent, {
        ...
        data: {
          ...
          description: this.translate.instant('memberDialog.publicKeyExistDialogDescription', { value: this.member.publicKey }),
        }
      }).afterClosed().toPromise();
      if (!ok) return;
    }

    const publicKey: string = await this.dialog.open(PromptDialogComponent, {
      width: '500px',
      data: {
        title: this.translate.instant('memberDialog.publicKey'),
    ...
    if (publicKey) {
      ...
      this.snack.open(this.translate.instant('app.savedSuccessfully'), '', { panelClass: 'success' }).onAction().subscribe(() => {
  ...
  async chargeWallet() {
    const charge: number = await this.dialog.open(PromptDialogComponent, {
      data: {
  ...
  async editStar() {
    const star = await this.dialog.open(PromptDialogComponent, {
      data: {
  ...
  removeMember() {
    this.dialog.open(AlertDialogComponent, {
      ...
      if (res) {
        ...
        this.snack.open(this.translate.instant('memberDialog.removedSuccessfully'), '', { panelClass: 'success' });
        this.dialogRef.close();
      }
    })
  }
