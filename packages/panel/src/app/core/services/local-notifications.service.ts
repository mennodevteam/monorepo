...
const SOUNDS = ['assets/sounds/notification_simple-01.ogg', 'assets/sounds/notification_simple-02.ogg'];
...
export class LocalNotificationsService {
  ...
  add(notif: LocalNotification): LocalNotification {
    if (notif.id) {
      const exist = this._notifications.find((x) => x.id === notif.id);
    ...
    notif.onClosed = this._onRemove
      .pipe(filter((x) => x === notif))
