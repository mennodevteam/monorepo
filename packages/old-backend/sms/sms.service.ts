...
export class SmsService {
      ...
      for (const kavenegarDto of kavenegarDtos) {
        kavenegarApi.SendArray(kavenegarDto, async (response, status) => {
          ...
          if (status == 200) {
            ...
            const savedSms = await this.smsRepository.save(sentSms);
            resolve(savedSms);
          } else {
            reject(response);
          }
  ...
  async filter(filter: FilterSmsDto): Promise<[Sms[], number]> {
    ...
    const sms = await this.smsRepository.findAndCount({
      where: condition,
  ...
  async handleCron() {
    ...
    const allSms = await this.smsRepository.find({
    ...
    while (allSms.length) {
      ...
      kavenegarApi.Status(body, (response, status) => {
        if (status === 200) {
          ...
          this.smsRepository.save(savedSms);
        }
      });
    }
  }
