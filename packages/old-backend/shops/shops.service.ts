...
import {
  ...
  SmsMicroservice,
  UserMicroservice,
} from 'src/core/microservices';
import { Club } from 'src/core/models/club';
import { CreateShopDto } from 'src/core/models/create-shop.dto';
import { Portal } from 'src/core/models/portal';
import { SmsAccount } from 'src/core/models/sms-account';
import { Menu } from 'src/menus/entities/menu';
import { Region } from 'src/regions/entities/region';
import { ShopUser } from 'src/shop-users/entities/shop-user';
import { ShopUserRole } from 'src/shop-users/entities/shop-user-role.enum';
import { ShopUsersService } from 'src/shop-users/shop-users.service';
import {
  FindConditions,
  FindManyOptions,
  In,
...
import { ShopPlugin } from '../plugins/entities/shop-plugin';
import { ShopPluginsService } from '../plugins/shop-plugins.service';
...
export class ShopsService {
  ...
  async findShopGroupByCode(code: string): Promise<ShopGroup> {
    const shopGroup = await this.shopGroupRepository.findOne(
      { code },
      {
        relations: ['shops'],
      }
  ...
  async privateInsert(dto: CreateShopDto) {
    ...
    if (!this.isUsernameValid(dto.loginUsername))
      throw new RpcException({
        code: 406,
        message: 'the loginUsername is invalid',
      });
    let existUser = await this.userClient
      .send('users/findOneByUsername', dto.loginUsername)
      .toPromise();
    if (existUser)
      throw new RpcException({
        code: 409,
        message: 'Duplicated Shop loginUsername',
      });

    if (dto.mobilePhone) {
      existUser = await this.userClient
        .send('users/findOneByPhone', dto.mobilePhone)
        .toPromise();
      if (existUser)
        throw new RpcException({
          code: 409,
          message: 'Duplicated Shop mobile phone',
        });
    }

    if (dto.username) {
      if (!this.isUsernameValid(dto.username))
        throw new RpcException({
          code: 406,
      ...
      if (existingShop)
        throw new RpcException({ code: 409, message: 'Duplicated Shop link' });
    }
    ...
    if (dto.prevServerCode) {
      ...
      if (existingShop)
        throw new RpcException({
          code: 409,
          message: 'Duplicated Shop prev sever code',
        });
    }
    if (dto.code) {
      const existingShop = await this.shopsRepository.findOne({
    ...
    if (dto.regionId) region = <Region>{ id: dto.regionId };
      ...
      region = await this.regionsRepository.save(<Region>{
    ...
    smsAccount = await this.smsClient
      .send<SmsAccount>('smsAccount/save', smsAccount)
      .toPromise();
    ...
    shop.region = region;
    shop.clubId = club.id;
    if (process.env.DEFAULT_BANK_PORTAL_ID)
      shop.bankPortalId = process.env.DEFAULT_BANK_PORTAL_ID;
  ...
  async insert(dto: CreateShopDto): Promise<any> {
    ...
    if (!dto.pluginKeys && !dto.pluginKeys.length)
      throw new RpcException({
        code: 403,
        message: 'plugin keys are required',
      });
    for (const key of dto.pluginKeys) {
      if (key) {
        ...
        if (splites.length !== 3)
          throw new RpcException({
            code: 403,
            message: 'plugin keys are invalid',
          });
    ...
    if (existingShop)
      throw new RpcException({
        code: 409,
        message: 'Duplicated Shop username',
      });

    if (
      dto.loginUsername != undefined &&
      !this.isUsernameValid(dto.loginUsername)
    ) {
      throw new RpcException({
        code: 406,
        message: 'the loginUsername is invalid',
    ...
    const existUser = await this.userClient
      .send('users/findOneByUsername', dto.loginUsername)
      .toPromise();
    if (existUser)
      throw new RpcException({
    ...
    for (const s of shops) {
      if (s.details.openingHours) {
        ...
        if (Shop.isOpen(s.details.openingHours, now)) {
          ...
          let differentTime;
          if (lastConnection)
            differentTime = now.valueOf() - lastConnection.valueOf();
          console.log(
            `${s.id} is open - the difference time is ${differentTime},now ${now}`
          );
          if (
            lastConnection === null ||
            Math.round(differentTime / 60000) > 10
          ) {
            console.log('this shop has condition for get sms -step 1', s);

            if (
              !s.lastDisconnectAlertAt ||
              (!s.connectionAt && !s.lastDisconnectAlertAt) ||
              new Date(s.lastDisconnectAlertAt).valueOf() <
                new Date(s.connectionAt).valueOf()
            )
              console.log(
                'this shop has Some condition for get Sms - step 2',
                s
              );

            {
              try {
                const shopUser: ShopUser[] = await this.shopUserService.get(
                  s.id
                );
                let mobilePhoneAdmin = shopUser.find(
                  (x) => x.role === ShopUserRole.Admin
                ).user.mobilePhone;
                let tokens: string[] = [];
                tokens[0] = '-';
                tokens[10] = s.title;
